...
CONF_MAX_CONSECUTIVE_FAILURES = "max_consecutive_failures" # 0 = disabled, otherwise reboot after N consecutive command failures
...
schema_dict = {
    ...
    cv.Optional(CONF_MAX_CONSECUTIVE_FAILURES): cv.uint16_t,
}
...
async def to_code(config):
    ...
    cg.add(
        var.load_polling_parameters(
            ...
            config.get(CONF_MAX_CONSECUTIVE_FAILURES),
        )
    )
