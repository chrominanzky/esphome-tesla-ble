...
if (ble_disconnected_ != BleConnected) // While disconnected update duration of disconnection
{
  publishSensor (NumericSensorId::BleDisconnectedTime, (millis() - ble_disconnected_time_) / 1000);
    ...
    void TeslaBLEVehicle::load_polling_parameters (const int post_wake_poll_time, const int poll_data_period,
                                                   ...
                                                   const int wake_on_boot, const int max_consecutive_failures)
    {
    ...
    void TeslaBLEVehicle::onCommandFailed()
    {
      consecutive_command_failures_++;
      ESP_LOGW(TAG, "Consecutive command failures: %d/%d", consecutive_command_failures_, max_consecutive_failures_);
      if (max_consecutive_failures_ > 0 && consecutive_command_failures_ >= max_consecutive_failures_)
      {
        ESP_LOGE(TAG, "Too many consecutive command failures (%d), restarting ESP..", consecutive_command_failures_);
        esp_restart();
      }
    }

    void TeslaBLEVehicle::onCommandSucceeded()
    {
      if (consecutive_command_failures_ > 0)
      {
        ESP_LOGI(TAG, "Command succeeded, resetting consecutive failure counter (was %d)", consecutive_command_failures_);
      }
      consecutive_command_failures_ = 0;
    }
