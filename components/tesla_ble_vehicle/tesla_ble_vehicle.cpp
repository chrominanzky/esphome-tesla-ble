    ...
    void TeslaBLEVehicle::process_response_queue()
    {
      ...
      if (read_queue_message_.has_signedMessageStatus)
      {
        if (read_queue_message_.signedMessageStatus.operation_status == UniversalMessage_OperationStatus_E_OPERATIONSTATUS_ERROR)
        {
          ...
          UniversalMessage_Domain error_domain = read_queue_message_.from_destination.sub_destination.domain;
          invalidateSession(error_domain);

          // For epoch errors, immediately request fresh session info to speed up recovery
          if (read_queue_message_.signedMessageStatus.signed_message_fault == UniversalMessage_MessageFault_E_MESSAGEFAULT_ERROR_INCORRECT_EPOCH)
          {
            ESP_LOGW(TAG, "Incorrect epoch for domain %s, requesting fresh session info",
                     domain_to_string(error_domain));
            sendSessionInfoRequest(error_domain);

            // Transition command to AUTH_RESPONSE state so handleSessionInfoUpdate
            // can directly move it to READY when the fresh session info arrives
            if (!command_queue_.empty())
            {
              BLECommand &cmd = command_queue_.front();
              if (error_domain == UniversalMessage_Domain_DOMAIN_VEHICLE_SECURITY &&
                  cmd.state == BLECommandState::WAITING_FOR_VCSEC_AUTH)
              {
                cmd.state = BLECommandState::WAITING_FOR_VCSEC_AUTH_RESPONSE;
                cmd.last_tx_at = millis();
              }
              else if (error_domain == UniversalMessage_Domain_DOMAIN_INFOTAINMENT &&
                       cmd.state == BLECommandState::WAITING_FOR_INFOTAINMENT_AUTH)
              {
                cmd.state = BLECommandState::WAITING_FOR_INFOTAINMENT_AUTH_RESPONSE;
                cmd.last_tx_at = millis();
              }
            }
          }
